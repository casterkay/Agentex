...
export const purchaseReceiptSchema = z.object({
  ...
  settlement_provider: z.literal("arkhai"),
  escrow_id: z.string().min(1),
  escrow_uid: z.string().min(1),
  escrow_transaction_hash: z.string().regex(/^0x[a-f0-9]{64}$/).optional(),
  fulfillment_uid: z.string().min(1).optional(),
  fulfillment_transaction_hash: z.string().regex(/^0x[a-f0-9]{64}$/).optional(),
  arbitration_status: z.enum(["not_requested", "requested", "approved", "rejected"]),
  arbitration_transaction_hash: z.string().regex(/^0x[a-f0-9]{64}$/).optional(),
  collection_status: z.enum(["not_collectible", "collectible", "collected", "refunded"]),
  collection_transaction_hash: z.string().regex(/^0x[a-f0-9]{64}$/).optional(),
